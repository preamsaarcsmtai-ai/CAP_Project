...
export class SuperAdminController {
    static async createInstitution(request, h) {
        ...
        const result = await SuperAdminService.addInstitution(name);
    ...
    static async createCollege(request, h) {
        const { institutionId, name } = request.payload;
        const result = await SuperAdminService.addCollege(institutionId, name);
        return h.response({ success: true, data: result}).code(201);
    }
